...
<template>
  <section class="space-y-4">
    ...
    <p v-if="isLoading" class="text-sm text-slate-600">Cargando torneos...</p>
    ...
    <div v-else class="overflow-hidden rounded-md border border-slate-200 bg-white">
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-for="tournament in tournaments" :key="tournament.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-sm text-slate-700">{{ tournament.start_date || '-' }}</td>
            <td class="px-4 py-3 text-sm text-slate-700">{{ tournament.end_date || '-' }}</td>
          </tr>
        </tbody>
      </table>
    </div>
